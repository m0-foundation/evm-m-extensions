...
coverage:
	FOUNDRY_PROFILE=$(profile) forge coverage --no-match-path 'test/integration/**/*.sol' --no-match-coverage UniswapV3SwapAdapter --no-match-coverage IndexingMath --report lcov && lcov --extract lcov.info -o lcov.info 'src/*' --ignore-errors inconsistent && genhtml lcov.info -o coverage
