...
contract MYieldFee is IMYieldFee, MExtension, YieldFee {
    ...
    uint128 public enableLatestMIndex;
     ...
     * @param rateOracle_       The address of a Rate Oracle contract.
     ...
     * @param admin             The address admnistrating the M extension. Can grant and revoke roles.
    ...
    function enableEarning() external override {
        ...
        emit EarningEnabled(enableLatestMIndex = _latestMIndex());
    ...
    function disableEarning() external override {
        ...
        // TODO: do we need to also store the MToken timestamp at which is was disabled?
        emit EarningDisabled(disableIndex = currentIndex());

        delete enableLatestMIndex;
    ...
    function currentIndex() public view virtual override returns (uint128) {
        ...
        if (enableLatestMIndex == 0) return disableIndex_;

        IContinuousIndexing mToken_ = IContinuousIndexing(mToken);

        // NOTE: Calculate the index with fee applied directly to the earner rate.
        //       Safe to use unchecked here, since `block.timestamp` is always greater than `latestUpdateTimestamp`.
        //       Cap the index to `type(uint128).max` to prevent overflow in present value math.
        unchecked {
            return
                UIntMath.bound128(
                    ContinuousIndexingMath.multiplyIndicesDown(
                        (UIntMath.safe128(uint256(disableIndex_) * mToken_.latestIndex()) / enableLatestMIndex),
                        ContinuousIndexingMath.getContinuousIndex(
                            ContinuousIndexingMath.convertFromBasisPoints(
                                UIntMath.safe32(
                                    (uint256(IRateOracle(rateOracle).earnerRate()) * (HUNDRED_PERCENT - yieldFeeRate)) /
                                        HUNDRED_PERCENT
                                )
                            ),
                            uint32(block.timestamp - mToken_.latestUpdateTimestamp())
                        )
                    )
                );
        }
    ...
    /// @dev Returns the latest index of the M Token.
    function _latestMIndex() internal view returns (uint128) {
        return IContinuousIndexing(mToken).latestIndex();
    }
